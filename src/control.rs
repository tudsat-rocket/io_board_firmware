...
use crate::index::{HcoId, PerHco, PerSensorSlot, PerTemp, PerValve, ValveId};
...
/// Rails and temperatures are two bounds on one type because on rev3 they are one ADC.
pub struct Control<R: RailSensing + TemperatureSensing = NoRails> {
    outputs: Outputs,
...
impl<R: RailSensing + TemperatureSensing> Control<R> {
